...
const run = async () => {
  ...
  const registryInfo = await connection.getAccountInfo(
    (
      await findRegistryAddress(rewarderKey)
    )[0]
  );
  if (!registryInfo) {
  ...
  const configMints: PublicKey[] = rewarderConfig?.quarries.map(
    (q: any) => new PublicKey(q.stakedToken.mint)
  );

  const mintsMinusConfig = [...(registry?.tokens ?? [])];
  ...
  const gaugeKeys = await Promise.all(
    allStakedTokenMints.map(
      throat(2, async (stakedTokenMint) => {
    ...
    await Promise.all(
      gaugeKeys.map(
        throat(2, async (gauge, i) => {
          ...
          if (
            !(await gaugeSDK.provider.connection.getAccountInfo(epochGauge))
          ) {
            return null;
          }
  ...
  const quarrySyncTXs = await Promise.all(
    chunk(
      ...
      throat(2, async (mints) => {
        return await rewarderW.syncQuarryRewards(mints);
      })
    )
  );

  const tx2 = await Promise.all(
    quarrySyncTXs.map(
      throat(2, (syncTX) =>
        sendTransactionWithRetry(
          connection,
